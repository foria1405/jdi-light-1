...
//    }
    ...
    private WebElement getContentPage() {
        WebElement el = null;
        try {
            String contentPage = getRefImage("Media");
            Thread.sleep(10000);
            el = getDriver().findElement(MobileBy.image(contentPage));
        } catch (Exception e) {
            e.printStackTrace();
        }
        return el;
    ...
    protected String getRefImage(String refImgName) throws Exception {
        ...
        File file = new File(System.getProperty("user.dir") + "\\" +refImgName + ".PNG");
